...
void *my_process_inc(int argc, char *argv[])
{
    ...
    {
        if (use_sem)
            usys_semWait(usys_findSem(SEM_ID));
        slowInc(&global, inc);
        if (use_sem)
            usys_semPost(usys_findSem(SEM_ID));
    }

    if (use_sem)
        usys_semClose(SEM_ID);
...
uint64_t test_sync(uint64_t argc, char *argv[])
{
    ...
    if (strcmp_u(argv[2], "0") == 0)
    {
        ...
        for (i = 0; i < TOTAL_PAIR_PROCESSES; i++)
        {
            if (usys_waitPid(pids[i]) == -1)
                printColor(RED, "ERROR WAITING FOR PROCESS (sem)\n");
    ...
    printColor(YELLOW, "SALIENDO del test_sync...\n");
    usys_myExit();
    return 0;
}